...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-dc-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Device Connector Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/deviceconnector/Policies/%s", moid), expectedJSONTemplate, vars)
}
